...
for df in [no_kernels, lv1_kernels, lv2_kernels]:
    ...
    svm_matrices.append(confusion_matrix(y_pred, y_test))
    svm_scores.append((int)(100*SVM.score(x_test,y_test))/100.0)
...
for i in range(3):
    axes[i][0].set_title(f'SVM/{PLT_TITLES[i]}/{svm_scores[i]}')
    axes[i][0].table(svm_matrices[i],loc='center')
    axes[i][0].get_xaxis().set_visible(False)
    axes[i][0].get_yaxis().set_visible(False)

f.tight_layout()
plt.savefig('svm_kernel_confusion.png')
